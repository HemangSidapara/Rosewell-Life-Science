...
class Logger {
  ...
  static void printLog({var tag = 'CPH Stocks', var printLog = '', var logIcon = 'ℹ️', bool isTimer = false}) {
    if (isTimer) {
      ...
      log(
        '|------------------------------>${Logger.logIcon} Response Time: $printLog seconds ${Logger.logIcon}<------------------------------|',
      );
    } else if (true) {
      Logger.logIcon = logIcon;
      Logger.tag = tag;
      log(
        '|---------->${Logger.logIcon} ${tag.contains('JSON METHOD') ? 'Request Data Start' : 'Response Data Start'} ${Logger.logIcon}<----------|',
      );
      log("$tag\t : $printLog", level: 2000);
      log(
        '|------------------------------>${Logger.logIcon} ${tag.contains('JSON METHOD') ? 'Request Data End' : 'Response Data End'} ${Logger.logIcon}<------------------------------|',
      );
    }
  }
}
